...
def run_fama_macbeth_by_size_group(df):
    ...
    输出：1. 每个市值组的专属PPT；2. 个股匹配所属组的预期收益率
    """
    print("\n" + "=" * 60)
    print("步骤2：分市值组Fama-MacBeth回归 计算组专属PPT(φ) 及 预期收益率(μ_t)")
    print("=" * 60)
    ...
    size_ppt_map = {}  # 存储：组名 -> PPT
    size_coefs_map = {}  # 存储：组名 -> 特征溢价系数
    ...
    for group_name in size_groups:
        ...
        if len(months) < 12:  # 该组有效月份不足1年，跳过
        ...
        group_ppt = -mean_coefs.get('log_prc', np.nan)
        ...
        size_coefs_map[group_name] = mean_coefs
        print(f"市值组【{group_name}】完成：PPT(φ) = {group_ppt:.6f}，有效回归月份：{len(monthly_reg_results)}")

        # 用该组专属系数计算该组个股的预期收益率
        group_data['expected_return'] = mean_coefs['const']
        for v in vars_model:
            group_data['expected_return'] += group_data[v] * mean_coefs[v]

        all_expected_return.append(group_data[['year_month', 'permno', 'size_group', 'expected_return']])

    # 合并结果
    expected_return_df = pd.concat(all_expected_return, ignore_index=True).dropna().reset_index(drop=True)

    print("\n" + "-" * 60)
    print(f"分市值组回归完成，有效组数量：{len(size_ppt_map)} 个")
    for g in size_ppt_map:
        print(f"  {g} 组 PPT: {size_ppt_map[g]:.6f}")
    print("-" * 60)

    return size_ppt_map, expected_return_df
...
def compute_benchmark_ur_and_elasticity(cov_results, expected_return_df, size_ppt_map, monthly_df):
    """
    【修改点5】适配分市值组PPT的弹性计算
    """
    print("\n" + "=" * 60)
    print("步骤4：基准模型均值-方差优化、UR计算与需求弹性提取（适配分市值组PPT）")
    ...
    for cov_item in tqdm(cov_results, desc="逐月优化与计算弹性"):
        ...
        # 过滤无效样本
        valid_mask = (~month_mu['expected_return'].isna()) & (~month_size_info['size_group'].isna())
        ...
        clean_size_group = month_size_info.loc[valid_mask, 'size_group'].values
        ...
        # 【修改点6】匹配持仓个股的市值组与对应PPT，计算个股弹性
        held_permnos = clean_permnos[pos_mask]
        held_size_group = clean_size_group[pos_mask]
        ...
        def get_size_group_stats(label):
            ...
            if np.sum(group_mask) <= 10:  # 组内持仓少于等于10只，当月该组结果无效
    ...
    for group in ['small', 'medium', 'large']:
        ...
        mask = results_df[n_col] <= 10#依旧小于等于10只
...
def main_benchmark_model():
    ...
    try:
        ...
        size_ppt_map, expected_return_df = run_fama_macbeth_by_size_group(monthly_df)
        ...
        # 4. OSQP 均值方差优化提取 UR 进而推算弹性
        results_df, detailed_df = compute_benchmark_ur_and_elasticity(cov_results, expected_return_df, size_ppt_map,
                                                                      monthly_df)

        # 保存详细的个股持仓记录（可选，方便你检查）
        detailed_df.to_csv(os.path.join(config['output_dir'], 'detailed_stock_level_ur.csv'), index=False)
