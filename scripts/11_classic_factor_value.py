...
MAX_ALLOWED_ELASTICITY=2000#存在极少数离谱数值，相当于做缩尾了。

# --------------------------
# 新增：多模型因子配置（完全满足FF3、CAPM、FF5需求）
# --------------------------
MODELS_CONFIG = {
    "FF6": ['MKT', 'SMB', 'HML', 'RMW', 'CMA', 'MOM']  # 按要求：仅保留六因子模型
}
...
def load_and_clean_data(monthly_data_path, factor_data_path, monthly_data_dir='.'):
    ...
    def filter_and_label_market_cap(group):
        # 【修改点1】不再剔除后20%，直接对全量股票分组
        valid_group = group.copy()

        # 【修改点2】分组规则改为：后30%=Small，30%-70%=Medium，前30%=Large
...
# ==========================================
# ==========================================
# 核心2：【修改点3】分市值组Fama-MacBeth回归计算PPT
# ==========================================
def run_fama_macbeth_by_size_group(df):
    """
    按市值分组(Small/Medium/Large)，每个组单独跑Fama-MacBeth回归
    输出：每个市值组的专属PPT系数映射表
    """
    print("\n" + "=" * 60)
    print("步骤2：分市值组Fama-MacBeth回归 计算组专属PPT(φ)")
    print("=" * 60)
    ...
    data = data.dropna(subset=['exret_next', 'size_group'] + vars_model)
    ...
    for group_name in size_groups:
        print(f"\n正在处理市值组：{group_name}")
        group_data = data[data['size_group'] == group_name].copy()
        months = sorted(group_data['year_month'].unique())

        if len(months) < 12:
            print(f"警告：市值组 {group_name} 有效月份不足12个月，已跳过")
            continue

        # 逐月截面回归
        monthly_reg_results = []
        for month in tqdm(months, desc=f"{group_name} Fama-MacBeth迭代"):
            month_data = group_data[group_data['year_month'] == month]
            if len(month_data) < 10:  # 当月该组至少10只股票
                continue
        ...
        if len(monthly_reg_results) < 6:
            print(f"警告：市值组 {group_name} 有效回归月份不足6个，已跳过")
            continue

        # 计算该组的PPT
        reg_results_df = pd.DataFrame(monthly_reg_results)
        mean_log_prc_coef = reg_results_df['log_prc'].mean()
        group_ppt = -mean_log_prc_coef

        if np.isnan(group_ppt) or group_ppt <= 0:
            print(f"警告：市值组 {group_name} PPT计算无效，已跳过")
            continue

        size_ppt_map[group_name] = group_ppt
        print(f"市值组【{group_name}】完成：PPT(φ) = {group_ppt:.6f}，有效回归月份：{len(monthly_reg_results)}")

    print("\n" + "-" * 60)
    print(f"分市值组PPT回归完成，有效组数量：{len(size_ppt_map)} 个")
    for g in size_ppt_map:
        print(f"  {g} 组 PPT: {size_ppt_map[g]:.6f}")
    print("-" * 60)
...
def compute_classic_model_elasticity(beta_df, factor_df, size_ppt_map, factor_cols, model_name, monthly_df):
    """
    【修改点4】适配分市值组PPT的弹性计算
    """
    print("\n" + "=" * 60)
    print(f"【{model_name}】步骤4：构建结构协方差、预期收益及计算需求弹性（适配分市值组PPT）")
    ...
    detailed_holdings = []
    # 市值-月度映射表
    stock_size_map = monthly_df[['year_month', 'permno', 'size_group']].drop_duplicates()
    ...
    for ym in tqdm(months, desc=f"{model_name} 逐月优化与计算弹性"):
        ...
        # 匹配市值分组信息
        month_size_info = stock_size_map[
            (stock_size_map['year_month'] == ym) &
            (stock_size_map['permno'].isin(valid_stocks))
            ].set_index('permno').reindex(valid_stocks)

        # 合并回 month_data 以确保顺序一致
        month_data = month_data.set_index('permno').join(month_size_info['size_group']).reset_index()
        ...
        # 【修改点5】匹配持仓个股的市值组与对应PPT，计算个股弹性
        held_permnos = valid_stocks[positive_mask]
        held_size_group = month_data.loc[positive_mask, 'size_group'].values

        # 逐个匹配市值组PPT
        held_ppt = np.array([size_ppt_map.get(g, np.nan) for g in held_size_group])
        valid_ppt_mask = ~np.isnan(held_ppt) & (held_ppt > 0) & (ur_hold > 0)

        # 过滤无效样本
        held_permnos = held_permnos[valid_ppt_mask]
        held_size_group = held_size_group[valid_ppt_mask]
        ...
        held_df = pd.DataFrame({
            'year_month': ym,
            'permno': held_permnos,
            'size_group': held_size_group,
            'weight': weights_hold,
            'tau': tau_hold,
            'ur': ur_hold,
            'size_group_ppt': held_ppt,
        ...
        # 分市值组统计
        def get_size_group_stats(label):
            group_mask = held_size_group == label
            if np.sum(group_mask) <= 10:
                return 0, np.nan, np.nan
            group_n = np.sum(group_mask)
            group_avg_ur = np.mean(ur_hold[group_mask])
            group_avg_elas = np.mean(stock_elasticity[group_mask])
            return group_n, group_avg_ur, group_avg_elas

        n_small, ur_small, elas_small = get_size_group_stats('Small')
        n_medium, ur_medium, elas_medium = get_size_group_stats('Medium')
        n_large, ur_large, elas_large = get_size_group_stats('Large')
        ...
        results.append({
            ...
            'n_hold_small': n_small,
            'avg_ur_small': ur_small,
            'demand_elasticity_small': elas_small,
            'n_hold_medium': n_medium,
            'avg_ur_medium': ur_medium,
            'demand_elasticity_medium': elas_medium,
            'n_hold_large': n_large,
            'avg_ur_large': ur_large,
            'demand_elasticity_large': elas_large
        })

    results_df = pd.DataFrame(results)
    for group in ['small', 'medium', 'large']:
        n_col = f'n_hold_{group}'
        ur_col = f'avg_ur_{group}'
        elas_col = f'demand_elasticity_{group}'
        mask = results_df[n_col] <= 10
        results_df.loc[mask, ur_col] = np.nan
        results_df.loc[mask, elas_col] = np.nan
    results_df = results_df[results_df['n_hold_total'] >= 10].reset_index(drop=True)

    # ===================== 新增：直接剔除极端大的弹性值（硬删除，非缩尾） =====================
    # 剔除全市场弹性超过阈值的月份 + 分市值弹性异常的月份
    extreme_mask = (
            (results_df['demand_elasticity_total'] > MAX_ALLOWED_ELASTICITY) |
            (results_df['demand_elasticity_small'] > MAX_ALLOWED_ELASTICITY) |
            (results_df['demand_elasticity_medium'] > MAX_ALLOWED_ELASTICITY) |
            (results_df['demand_elasticity_large'] > MAX_ALLOWED_ELASTICITY)
    )
    # 直接删除极端行，不保留
    ...
    print(
        f"【全市场】 平均UR: {results_df['avg_monthly_ur_total'].mean():.6f} | 平均弹性: {results_df['demand_elasticity_total'].mean():.2f}")
    print(
        f"【小市值】 平均UR: {results_df['avg_ur_small'].mean():.6f} | 平均弹性: {results_df['demand_elasticity_small'].mean():.2f}")
    print(
        f"【中市值】 平均UR: {results_df['avg_ur_medium'].mean():.6f} | 平均弹性: {results_df['demand_elasticity_medium'].mean():.2f}")
    print(
        f"【大市值】 平均UR: {results_df['avg_ur_large'].mean():.6f} | 平均弹性: {results_df['demand_elasticity_large'].mean():.2f}")
    print("=" * 60)
    ...
    return results_df, avg_ur, avg_elasticity, detailed_df
...
def main_classic_factor_models():
    ...
    try:
        ...
        size_ppt_map = run_fama_macbeth_by_size_group(monthly_df)
        if len(size_ppt_map) == 0:
            raise ValueError("无有效市值组的PPT计算结果，请检查数据")
        ...
        for model_name, factor_cols in MODELS_CONFIG.items():
            ...
            results_df, avg_ur, avg_elasticity, detailed_df = compute_classic_model_elasticity(
                beta_df, factor_df, size_ppt_map, factor_cols, model_name, monthly_df
            )
            # 5. 模型结果落盘
            results_df.to_csv(os.path.join(model_output_dir, f'value_{model_name}_elasticity_results.csv'), index=False)
            beta_df.to_csv(os.path.join(model_output_dir, f'value_{model_name}_betas.csv'), index=False)
            detailed_df.to_csv(os.path.join(model_output_dir, f'value_{model_name}_detailed_holdings.csv'), index=False)
